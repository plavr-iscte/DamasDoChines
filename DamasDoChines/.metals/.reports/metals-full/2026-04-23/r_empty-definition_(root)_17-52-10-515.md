error id: file://<WORKSPACE>/src/Engine.scala:
file://<WORKSPACE>/src/Engine.scala
empty definition using pc, found symbol in pc: 
empty definition using semanticdb
empty definition using fallback
non-local guesses:
	 -scala/collection/parallel/CollectionConverters.decideToContinue.
	 -scala/collection/parallel/CollectionConverters.decideToContinue#
	 -scala/collection/parallel/CollectionConverters.decideToContinue().
	 -decideToContinue.
	 -decideToContinue#
	 -decideToContinue().
	 -scala/Predef.decideToContinue.
	 -scala/Predef.decideToContinue#
	 -scala/Predef.decideToContinue().
offset: 3858
uri: file://<WORKSPACE>/src/Engine.scala
text:
```scala
package code
...
case class Engine() {
    ...
    def playRandomly(board: Board, r: MyRandom, player: Stone, lstOpenCoords: List[Coord2D], f: (List[Coord2D], MyRandom) => (Coord2D, MyRandom)): (Option[Board], MyRandom, List[Coord2D], Option[Coord2D]) = {
        if lstOpenCoords.isEmpty then { (None, r, lstOpenCoords, None) }
            ...
            fBoard match {
                case Some(newBoard) =>
                    val (decideT@@oContinue, rsN) = rs.nextInt(2)
