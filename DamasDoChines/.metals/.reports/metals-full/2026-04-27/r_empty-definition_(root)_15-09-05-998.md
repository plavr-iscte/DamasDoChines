error id: file://<WORKSPACE>/src/Cli.scala:scala/Any#toString().
	...
	def getCommand(prompt:String, command: String) = {
		...
		result.split("\\s+") match {
			...
			case "play" :: colFrom :: rowFrom :: colTo :: rowTo :: Nil =>
				engine.play(state.board, state.player, stringToCoord(colFrom.toString, rowFrom.toString), stringToCoord(colTo.to@@String, rowTo))
			case "pr" :: Nil => None
			case _ => None
		}
