error id: file://<WORKSPACE>/src/Main.scala:asInstanceOf.
file://<WORKSPACE>/src/Main.scala
empty definition using pc, found symbol in pc: asInstanceOf.
empty definition using semanticdb
empty definition using fallback
non-local guesses:
	 -scala/collection/parallel/CollectionConverters.entries.asInstanceOf.
	 -scala/collection/parallel/CollectionConverters.entries.asInstanceOf#
	 -scala/collection/parallel/CollectionConverters.entries.asInstanceOf().
	 -entries/asInstanceOf.
	 -entries/asInstanceOf#
	 -entries/asInstanceOf().
	 -scala/Predef.entries.asInstanceOf.
	 -scala/Predef.entries.asInstanceOf#
	 -scala/Predef.entries.asInstanceOf().
offset: 3835
...
def initboard(x:Int,y:Int,r:MyRandom): (Board,MyRandom,List[Coord2D])={
    ...
    val (y2, rd) = ry.nextInt
    val y_rem = Math.abs(y2)%y
    val (dr, rfinal) = rd.nextInt
    ...
    (entries.asInst@@anceOf[Board], rfinal.asInstanceOf[MyRandom], openCoords)
