error id: file:///C:/Users/migue/Documents/DamasDoChines/DamasDoChines/src/T1.scala:
...
def play(board:Board, player: Stone,coordFrom:Coord2D,coordTo:Coord2D,lstOpenCoords:List[Coord2D]):(Option[Board],List[Coord2D]) = {
    if( (coordTo._1-coordFrom._1) %2 !=0 || (coordTo._2-coordFrom._2) %2 !=0 || lstOpenCoords.contains(coordTo)) then
        (None,lstOpenCoords)
    def verCap(b:Board,c:Coord2D,lst:L):(Boolean,Board,List[Coord2D])={
        def next(cFrom:Coord2D:(Coord2D)={
            if(cFrom._1<c.coordTo._1) 
                return Coord2D(cFrom._1+1,cFrom._2)
            else if (cFrom._1>c.cT@@o._1) 
                return Coord2D(cFrom._1-1,cFrom._2)
            else ={
                if(cFrom._2<c.cTo._2) 
                    return Coord2D(cFrom._1,cFrom._2+1)
                else if (cFrom._2>c.cTo._2) 
                    return Coord2D(cFrom._1,cFrom._2-1)
            }
        }
        val cnext=next(c)
